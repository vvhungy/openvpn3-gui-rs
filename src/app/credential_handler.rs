//! Credential request/submit flow
    ...
    let on_cancel = move || {
        ...
        glib::spawn_future_local(async move {
            super::session_ops::disconnect_with_message(
                ...
                &format!("Authentication cancelled for '{}'. Session disconnected.", cn),
            )
            .await;
        });
    };
...
pub(crate) async fn request_challenge(
    ...
    // Collect challenge slots: (type, group, id, label, description)
...
async fn submit_challenge(
    ...
    for (att_type, group, id) in slots {
        session.UserInputProvide(*att_type, *group, *id, response).await?;
        info!("Provided challenge response for slot on session {}", session_path);
    }
