...
impl Application {
    pub fn run(args: AppArgs) -> anyhow::Result<ExitCode> {
        ...
        gtk_app.connect_startup(move |gtk_app| {
            ...
            // Periodic timer — refreshes tooltip duration and polls connection
            // statistics for connected sessions. Re-reads the interval each tick
            // so preference changes take effect.
            let tray_for_timer = tray_handle_clone.clone();
            let settings_for_timer = settings_clone.clone();
            let dbus_for_stats = dbus_conn.clone();
            glib::spawn_future_local(async move {
                loop {
                    let secs = settings_for_timer.tooltip_refresh_interval();
                    glib::timeout_future_seconds(secs).await;

                    // Poll statistics for connected sessions
                    ...
                    for (path, connected) in session_paths {
                        ...
                        if let Ok(obj_path) =
                            ...
                            && let Ok(builder) =
                                crate::dbus::session::SessionProxy::builder(&dbus_for_stats)
                                    .path(obj_path)
